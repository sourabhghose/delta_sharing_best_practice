...
The recipient workspace (`fevm-energy-copilot`) creates a provider reference and a foreign catalog to access shared data, then creates a local catalog for the medallion pipeline.

## Prerequisites

- Admin access on `fevm-energy-copilot`
- Unity Catalog enabled
- Provider has completed setup (notebook 01)
- Provider's sharing identifier available

## Steps

### 1. Create Provider

The provider object points to the sharing workspace:

```sql
CREATE PROVIDER IF NOT EXISTS e2_demo_west_provider
USING ID '<provider-sharing-identifier>';
```

Verify available shares:
```sql
SHOW SHARES IN PROVIDER e2_demo_west_provider;
```

### 2. Create Foreign Catalog

A foreign catalog provides read-only access to the shared tables:

```sql
CREATE CATALOG IF NOT EXISTS shared_energy_market
USING SHARE e2_demo_west_provider.energy_market_share;
```
...
- The foreign catalog reflects the current state of the provider's shared tables
